...
{
    public class EarthquakeX : IEarthquake
    {
        ...
        public EarthquakeX(float amount)
        {
            _timer = 0;
            _amount = amount;
            _isFinished = false;
        ...
        public EarthquakeX(float amount, float duration, float speed)
        {
            _timer = 0;
            _amount = amount;
            _isFinished = false;
        ...
        public void Update(GameTime gameTime)
        {
            ...
            if (!_isFinished)
            {
                ...
                if (_amount > 0)
                {
                    _translation.X = (int)(Math.Sin(_timer) * _amount);
                }
                else
                {
                    _isFinished = true;
